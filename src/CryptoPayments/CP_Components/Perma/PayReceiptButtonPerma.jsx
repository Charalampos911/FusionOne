...
export default function PayReceiptButtonPerma({ signer, PermaProductTransactionId }) {
  ...
  const handlePayReceipt = async (dispatch, apiRequest, PermaProductTransactionId) => {
    ...
    dispatch(
      ...
      .then(async (Response) => {
        ...
        const Total = APICorrectData.Transaction.Total; // e.g. "440.00" string
        const ReceiptGuid = APICorrectData.Transaction.Id;
        ...
        try {
          ...
          const txResponse = await callContractFunction(
            ...
            (messageCallback) =>{ alert("messageCallback===> "+messageCallback)}
          );
  ...
  return (
      <FormButton text= {loading ? "Processing..." : "Pay Invoice"} onClick={() => !loading && contract?handlePayReceipt(dispatch, apiRequest, PermaProductTransactionId):null}/>
  );
}