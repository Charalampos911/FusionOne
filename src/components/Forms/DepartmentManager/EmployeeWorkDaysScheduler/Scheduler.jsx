...
import { apiRequest,setDivisionId,clearEmployeeWorkDays } from '../../../../Redux/features/ApiReducer';
...
export default function Scheduler(props) { 
  ...
  useEffect(() => {
    dispatch(clearEmployeeWorkDays());
    if(Employee==null)return;
    dispatch(
      apiRequest({
        flatten: true,
        name: "Employees.Scheduler.jsx | useEffect",
        url: "api/EmployeeWorkDays/WorkMonthByWeeks",
        method: "POST",
        body: {Month:UserDate,EmployeeId: Employee},
        auth: true,
        tokenRequired: true,
        storeIn: "EmployeeWorkDays"
      })
    )
   }, [Employee,UserDate,ActiveTab]);
  ...
  <div id="Solo-Scheduler-container">
    <div className="Solo-Scheduler">
      <div className='Solo-Scheduler-head'>     
        ...
        <label>Employee Work Days Scheduler</label>
      </div>
      <div className='Scheduler-box'>
        ...
        <QuerySelect 
          isDynamic={true}
          optionsArray={Employees? Employees:[]}
          placeholder="Employee"
          setValue={(val) => (setEmployee(val?.Id),setScope(false))}
          zIndex={2}
        />
        </div>
      </div>
      ...
      <div className='Scheduler-week-box'>
        {Api.EmployeeWorkDays?.map((item, indexA) =>{ 
          var ThisWeek = item;
