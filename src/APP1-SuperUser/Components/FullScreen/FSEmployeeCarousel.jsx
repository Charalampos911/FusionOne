...
export const FSEmployeeCarousel = ({ Employees = [], onSelect }) => {
  const Api = useSelector((state) => state.Api);
  const dispatch = useDispatch();

  // Store current active image index per employee card: { [Id]: imageIndex }
  const [activeImageIndexes, setActiveImageIndexes] = useState({});

  // Lightbox modal state: null or { item: employeeObject, index: number }
  const [lightboxState, setLightboxState] = useState(null);

  // Card image navigation handlers
  const handleNextImage = (e, itemId, totalImages) => {
    e.stopPropagation(); // Prevents triggering employee selection
  ...
  return (
    <div className="org-gallery-container fullscreen">
      {/* <h2 className="org-gallery-title">FusionOne Employees</h2> */}
